...
class RoadmapHaltonOffLLDens : public Roadmap<RoadmapArgs>
{
...
public:
   RoadmapHaltonOffLLDens(RoadmapArgs & args):
      Roadmap<RoadmapArgs>(args, "HaltonOffLLDens", 0),
      _dim(0),
      _bounds(0),
      _num_per_batch(0),
      ...
      _offset_state(this->space)
   {
      ...
      if (this->space->getType() != ompl::base::STATE_SPACE_REAL_VECTOR)
         throw std::runtime_error("RoadmapHaltonOffLLDens only supports rel vector state spaces!");
      ...
      this->template declareParam<unsigned int>("num_per_batch", this,
         &RoadmapHaltonOffLLDens<RoadmapArgs>::setNumPerBatch,
         &RoadmapHaltonOffLLDens<RoadmapArgs>::getNumPerBatch);
      this->template declareParam<double>("radius_first_batch", this,
         &RoadmapHaltonOffLLDens::setRadiusFirstBatch,
         &RoadmapHaltonOffLLDens::getRadiusFirstBatch);
      this->template declareParam<unsigned int>("seed", this,
         &RoadmapHaltonOffLLDens::setSeed,
         &RoadmapHaltonOffLLDens::getSeed);
   }
   ...
   void initialize()
   {
      ...
      double base = log(log(_num_per_batch)) / ((double)_num_per_batch);
      _gamma = _radius_first_batch / pow(base, 1./_dim);
   ...
   {
      double num = (i_batch+1) * _num_per_batch;
      return _gamma * pow(log(log(num)) / num, 1./_dim);
   }
