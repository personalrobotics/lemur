...
or_multiset::CCTimer::CCTimer(OpenRAVE::EnvironmentBasePtr env):
   OpenRAVE::PlannerBase(env), env(env)
{
   __description = "E8 roadmap planner";
   RegisterCommand("GetTimes",
      boost::bind(&or_multiset::CCTimer::GetTimes,this,_1,_2),
      "get timing information from last plan");
}

or_multiset::CCTimer::~CCTimer()
{
}

bool
or_multiset::CCTimer::InitPlan(OpenRAVE::RobotBasePtr robot, std::istream & inparams_ser)
{
...
bool
or_multiset::CCTimer::InitPlan(OpenRAVE::RobotBasePtr inrobot, OpenRAVE::PlannerBase::PlannerParametersConstPtr params)
{
   robot = inrobot;
...
or_multiset::CCTimer::GetParameters() const
{
   return OpenRAVE::PlannerBase::PlannerParametersConstPtr();
}

OpenRAVE::PlannerStatus
or_multiset::CCTimer::PlanPath(OpenRAVE::TrajectoryBasePtr traj)
{
...
bool or_multiset::CCTimer::GetTimes(std::ostream & sout, std::istream & sin) const
{
#if 0
   sout << "checktime " << boost::chrono::duration<double>(ompl_checker->dur_checks).count();
   sout << " totaltime " << 0.0;
   sout << " n_checks " << ompl_checker->num_checks;
   sout << " e8_dur_total " <<  ompl_planner->as<ompl_multiset::CCTimer>()->getDurTotal();
   sout << " e8_dur_roadmapgen " <<  ompl_planner->as<ompl_multiset::CCTimer>()->getDurRoadmapGen();
   sout << " e8_dur_roadmapinit " <<  ompl_planner->as<ompl_multiset::CCTimer>()->getDurRoadmapInit();
   sout << " e8_dur_lazysp " <<  ompl_planner->as<ompl_multiset::CCTimer>()->getDurLazySP();
   sout << " e8_dur_search " <<  ompl_planner->as<ompl_multiset::CCTimer>()->getDurSearch();
   sout << " e8_dur_eval " <<  ompl_planner->as<ompl_multiset::CCTimer>()->getDurEval();
#endif
   return true;
}
